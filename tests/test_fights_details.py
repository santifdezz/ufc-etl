...
def test_fights_details_scraper_runs(tmp_path):
    ...
    with open(src, encoding='utf-8') as fsrc, open(dst, 'w', encoding='utf-8', newline='') as fdst:
        ...
        for i, row in enumerate(reader):
            ...
            if i == 10:
                break
    extract_event_fights_details(dst)
    details_path = os.path.join(test_data_dir, 'raw_event_fights_details.csv')
    rows, fieldnames = get_first_n_rows(details_path)
    assert len(rows) > 0
    assert len(fieldnames) >= 10
